...
## 2. Performance Analysis (NiaH 40,000 Tokens)
Based on benchmarks with Qwen 3.5 1.5B/2B (MLA architecture):

| Strategy | Latency (ms/layer) | Cosine Similarity (Fidelity) | Speedup |
| :--- | :---: | :---: | :---: |
| **Gaussian Baseline** | ~15.52 ms | 0.9940 | 1.00x |
| **Kudahitam-Quant** | **~1.25 ms** | **0.9938** | **12.4x Faster** |

## 3. Why FWHT is Superior?
In the original `compressors.py`, every KV-cache compression required a large $D \times D$ matrix multiplication. This not only consumed GPU compute cycles but also introduced significant memory-access overhead for the large projection matrix.
...
3. **Scaling**: While Gaussian scaling is quadratic, FWHT scaling is near-linear, making it the only viable choice for future models with $D=1024$ or $D=2048$.
